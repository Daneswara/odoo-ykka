...
class KaAccountPaymentConfirmWizard(models.TransientModel):
    _inherit = "ka_account.payment.confirm.wizard"
        ...
        if active_ids:
            ...
            for payment in self.env['ka_account.payment'].browse(active_ids):
                ...
                if payment.amount_dpp == 0:
                    raise UserError('Nilai DPP tidak boleh 0')
                ...
                if payment.amount_pph > 0:
                    #Fitur baru setelah koreksi bahwa pph bisa lebih dari satu perkiraan
                    if len(payment.pph_ids) > 0:
                        for pph in payment.pph_ids:
                            vals_pph = {
                                'amount' : (pph.amount)*-1,
                                'name' : (_('PPH: %s; %s') % (payment.no_kwitansi, payment.vendor_invoice_date)),
                                'account_id' : pph.tax_id.account_id.id,
                                'ka_payment_id': payment.id,
                            }
                            line_items.append((0, 0, vals_pph))
                    else:
                        #untuk menakomodir tagihan sebelum revisi 
                        vals_pph = {
