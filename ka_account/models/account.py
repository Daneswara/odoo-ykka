...
class AccountAccount(models.Model):
    ...
    account_template_id = fields.Many2one('account.account.template', 'Account Template')
    ...
    report_line_ids = fields.Many2many('account.financial.report', 'account_account_financial_report', 'account_id', 'report_line_id', 'Group Laporan')
...
class AccountJournal(models.Model):
    ...
    def _prepare_liquidity_account(self, name, company, currency_id, type):
        ...
        liquidity_type = self.env.ref('ka_account.data_account_type_view')
...
class AccountPayableCategory(models.Model):
    _name ="ka_account.payable.category"
    _description = "Perkiraan Kategori Pembelian"
    _order = "code"

    code = fields.Char("Kode", size=4, index=True)
    name = fields.Char("Keterangan")
    property_account_payable_id = fields.Many2one('account.account', company_dependent=True, required=True, string="Perkiraan Hutang", domain="[('internal_type', '=', 'payable'), ('deprecated', '=', False)]")
